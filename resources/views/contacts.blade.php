@extends('layouts.app')
@push('head')
    <link rel="stylesheet" href="{{ asset('css/contacts-page.css') }}">
@endpush
@section('title')
    Контакты
@endsection
@section('content')
    <section class="contacts-page">
        <div class="container">
            <div class="links">
                <a href="{{ route('index') }}"><span>Главная</span></a>
                <img src="../img/vacancy-arrow.png" alt="">
                <a href="{{ route('contacts') }}"><span class="active-link">Контакты</span></a>
            </div>
            <h1 class="section-h1">
                Контакты
            </h1>
            <div class="container-drop">
                <div>
                    <div class="phone">
                        <h5>Телефон</h5>
                        <a href="tel:8(800)267-19-45">8(800)267-19-45</a>
                    </div>
                    <div class="contacts_list">
                        <div class="left-block">
                            <div class="contact_item">
                                <h5>Общие вопросы</h5>
                                <a href="mailto:info@meritocratconsulting.ru">info@meritocratconsulting.ru</a>
                            </div>
                            <div class="contact_item">
                                <h5>Пресс-служба</h5>
                                <a href="mailto:pr@meritocratconsulting.ru">pr@meritocratconsulting.ru</a>
                            </div>
                            <div class="contact_item">
                                <h5>Партнерство</h5>
                                <a href="mailto:partner@meritocratconsulting.ru">partner@meritocratconsulting.ru</a>
                            </div>
                            <div class="contact_item">
                                <h5>Подписка</h5>
                                <a href="mailto:order@meritocratconsulting.ru">order@meritocratconsulting.ru</a>
                            </div>
                            <div class="contact_item">
                                <h5>Центр поддержки</h5>
                                <a href="mailto:support@meritocratconsulting.ru">support@meritocratconsulting.ru</a>
                            </div>
                        </div>
                        <div class="right-block">
                            <div class="contact_item">
                                <h5>Отдел маркетинга</h5>
                                <a href="mailto:info@meritocratconsulting.ru">info@meritocratconsulting.ru</a>
                            </div>
                            <div class="contact_item">
                                <h5>Отдел кадров</h5>
                                <a href="mailto:hr@meritocratconsulting.ru">hr@meritocratconsulting.ru</a>
                            </div>
                            <div class="contact_item">
                                <h5>Юридический отдел</h5>
                                <a href="mailto:lawyer@meritocratconsulting.ru">lawyer@meritocratconsulting.ru</a>
                            </div>
                            <div class="contact_item">
                                <h5>Тендерный отдел</h5>
                                <a href="mailto:tender@meritocratconsulting.ru">tender@meritocratconsulting.ru</a>
                            </div>
                            <div class="contact_item">
                                <h5>Бухгалтерия</h5>
                                <a href="mailto:buh@meritocratconsulting.ru">buh@meritocratconsulting.ru</a>
                            </div>
                        </div>
                    </div>
                    <div class="contacts-application">
                        <button class="button-white" onclick="showModal()">Написать сообщение</button>
                        <div class="contacts-modal">
                            <img width="300px" src="/img/contacts-form.png">
                            <div class="left-side">
                                <h2>Хочу работать с вами</h2>
                                <div class="message">Оставьте заявку и в скором времени
                                    мы с вами свяжемся </div>
                            </div>
                            <div class="right-side">
                                <svg onclick="hideModal()" width="20" height="20" viewBox="0 0 40 41" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                    <mask id="mask0_1261_4472" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="40" height="41">
                                        <rect y="0.666992" width="40" height="40" fill="url(#pattern0_1261_4472)"/>
                                    </mask>
                                    <g mask="url(#mask0_1261_4472)">
                                        <rect y="0.666992" width="40" height="40" fill="white"/>
                                    </g>
                                    <defs>
                                        <pattern id="pattern0_1261_4472" patternContentUnits="objectBoundingBox" width="1" height="1">
                                            <use xlink:href="#image0_1261_4472" transform="scale(0.00195312)"/>
                                        </pattern>
                                        <image id="image0_1261_4472" width="512" height="512" xlink:href="data:image/png;base64,"/>
                                    </defs>
                                </svg>
                                <form action="{{ route('application.store') }}" method="POST" class="right-column" enctype="multipart/form-data">
                                    @csrf
                                    @method('POST')
                                    <div class="contacts-forms">
                                        <input type="text" name="name" placeholder="Имя" class="contacts-form">
                                        <input type="text" name="phone" placeholder="Телефон" class="contacts-form">
                                        <input type="email" name="email" placeholder="Email" class="contacts-form">
                                        <input type="hidden" name="source" value="contacts">
                                        <textarea name="comment" id="comment" cols="30" rows="10" class="contacts-comments" placeholder="Комментарии"></textarea>
                                        <span class="text-data">*Нажимая кнопку «Отправить», я даю согласие на обработку <a>персональных данных</a></span>
                                        <div class="form-btns">
                                            <input type="file" class="button-with-border" name="file" value="Прикрепить файлы">
                                            <button class="button-white">Отправить</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                    <div class="gradient-blue"></div>
                    <div class="gradient-orange"></div>
                    <div class="contacts-address">
                        <h5>Адрес</h5>
                        <p>115230, г. Москва, проезд Хлебозаводский, д. 7, с 9</p>
                        <img src="/img/map.png" alt="адрес">
                    </div>
                </div>
            </div>
        </div>
    </section>
@endsection
@push('script')
    <script>
        function showModal() {
            $(".contacts-modal").css('display', 'flex');
        }
        function hideModal() {
            $('.contacts-modal').hide();
        }
    </script>
    <script src="{{ asset('js/dropdown-projects.js') }}"></script>
    <script src="{{ asset('js/dropdown-vacancy.js') }}"></script>
@endpush
